...
import { Mic, Square, Languages, AlertCircle, Volume2 } from "lucide-react";
import { useLiveInterpreterSession } from "../hooks/useLiveInterpreterSession";
import { SCENARIO_TITLE, LANGUAGE_PAIR } from "../lib/liveConfig";
    ...
    <div className="max-w-4xl mx-auto px-2 py-2">
      ...
      <div className="mb-8">
        ...
        <p className="text-zinc-400 text-sm mt-1">
          NAATI-style dialogue interpreting · {LANGUAGE_PAIR}
        </p>
      </div>
      ...
      <div className="bg-zinc-950 border border-zinc-800/80 rounded-2xl p-8 flex flex-col items-center">
        ...
        {!isActive ? (
          <button
            onClick={start}
            className="px-6 py-3 rounded-xl bg-accent-dark hover:bg-accent text-white font-bold text-sm transition-colors cursor-pointer flex items-center gap-2 shadow-lg shadow-accent-dark/20"
          >
      ...
      {status === "idle" && (
        <p className="text-zinc-500 text-xs text-center mt-4 max-w-md mx-auto">
          The examiner reads one segment, then a beep signals your turn —
          interpret it aloud into the other language. Another beep, and the
          examiner continues. Allow microphone access when prompted.
        </p>
      )}
